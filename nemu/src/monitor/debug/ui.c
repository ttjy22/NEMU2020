...
#define CAL_HEC {int j=0, base = 0;for (; tokens[++i].str[j]; ++j) {if (tokens[++i].str[j] >= 'a' && tokens[++i].str[j] <= 'f')base = tokens[i].str[j] - 'a' + 10 + base * 16;else base = tokens[i].str[j] - '0' + base * 16;stk_n[++t_n] = base;}}
